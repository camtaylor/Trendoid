...
public class TrendAdapter extends ArrayAdapter<Trend> {
	...
	/***
	 * Calls the super constructor and saves the adapter's context as well as the model data (i.e. the courses).
	 * 
	 * @param context
	 * @param trends
	 */
	...
	/**
	 * getView()'s job is to return a View that has been filled with data for the row given by position.
	 */
	public View getView(int position, View convertView, ViewGroup parent) {
		...
		//LinearLayout trendLayout = (LinearLayout) trendView.findViewById(R.id.trendLayout);

		// Display random image
		ImageView imageView = (ImageView) trendView.findViewById(R.id.imageView);
		if (trend.getSource() == "google")
			imageView.setImageResource(R.drawable.googlelogo);
		else if(trend.getSource() == "aol")
			imageView.setImageResource(R.drawable.aollogo);
		...
		SharedPreferences prefs = context.getSharedPreferences("pref_general", 0);
